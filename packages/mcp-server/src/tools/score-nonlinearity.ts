...
export function scoreNonLinearity(
  ...
  if (maxChoices > 7) cognitiveFriendlyScore = 40;
  else if (maxChoices > 5) cognitiveFriendlyScore = 60;
  else if (maxChoices > 3) cognitiveFriendlyScore = 80; // 3-5 个选项略多但可接受
  else cognitiveFriendlyScore = 100; // 1-3 个选项最佳

  const overallScore = Math.min(
    100,
    Math.round(pathScore * 0.4 + distScore * 0.3 + diversityScore * 0.2 + cognitiveFriendlyScore * 0.1)
  );
