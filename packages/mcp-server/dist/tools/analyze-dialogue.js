...
export function analyzeDialogue(input) {
    ...
    const stats = nodes.map((n) => ({
        ...
        totalChars: (n.dialogues || []).reduce((sum, d) => sum + (d.text?.length || 0), 0),
    }));
    ...
    const characterStats = Array.from(characterMap.entries()).map(([name, data]) => ({
        characterName: name,
    ...
    const qualityScore = Math.max(0, Math.min(100, 70 - shortPenalty - longPenalty - narrationPenalty + characterBonus + emotionBonus));
    return {
...
//# sourceMappingURL=analyze-dialogue.js.map